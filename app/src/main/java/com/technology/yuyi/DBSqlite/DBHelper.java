 ...
 *
 *         创建meituan_cities.db数据库，这个数据库中的city表 包含全部城市
 *
 */
public class DBHelper extends SQLiteOpenHelper {
	// 用户数据库文件的版本
	private static final int DB_VERSION = 3;
	// 数据库文件目标存放路径为系统默认位置，com.technology.yuyi 是你的包名
	private static String DB_PATH = "/data/data/com.technology.yuyi/databases/";
	/*
	 * //如果你想把数据库文件存放在SD卡的话 private static String DB_PATH =
	 * android.os.Environment.getExternalStorageDirectory().getAbsolutePath() +
	 * "/arthurcn/drivertest/packfiles/";
	 */
	private static String DB_NAME = "meituan_cities.db";//数据库名
	private static String ASSETS_NAME = "meituan_cities.db";//将assets下的meituan_cities.db这个数据库读到自己创建的 DB_NAME = "meituan_cities.db"中
	...
	public void createDataBase() throws IOException {
		...
		if (dbExist) {
			...
			try {
				...
				if (!dir.exists()) {//判断这个DB_PATH文件夹是否存在
					dir.mkdirs();
				}
				File dbf = new File(DB_PATH + DB_NAME);
				if (dbf.exists()) {//判断数据库DB_NAME = "meituan_cities.db"是否存在
					dbf.delete();
				}
				//创建数据库meituan_cities.db
